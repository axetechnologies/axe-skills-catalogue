"""Upstream catalog provider for the federation tier.
...
def _base() -> str:
    """The upstream API base. Required: no default is hardcoded here, so the
    vendor address lives in the environment, not in this repo."""
    b = os.environ.get("AXE_UPSTREAM_BASE", "").rstrip("/")
    if not b:
        raise RuntimeError("AXE_UPSTREAM_BASE is unset: the upstream catalog base URL must be configured")
    return b



# The upstream aggregates other registries. `built-in` is its own; the rest are
...
class AxeCatalog:
    ...
    def __init__(self, *, skills_url: str | None = None, meta_url: str | None = None) -> None:
        self._skills_url = skills_url or os.environ.get("AXE_UPSTREAM_SKILLS") or f"{_base()}/skills.json"
        self._meta_url = meta_url or os.environ.get("AXE_UPSTREAM_META") or f"{_base()}/skills-meta.json"

    def manifest(self) -> AxeManifest:
    ...
    def fetch(self, limit: int | None = None, page_size: int = 0) -> Iterator[CommunityEntry]:
        ...
        try:
            ...
            raise RuntimeError(f"upstream catalog unreachable: {exc}") from exc
    ...
    def _homepage(self, row: dict) -> str:
        ...
        return f"{_base().rsplit('/docs/', 1)[0]}/{docs}" if docs else ""
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Inspect the upstream catalog without ingesting.")
